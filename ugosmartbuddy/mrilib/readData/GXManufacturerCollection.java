...
public class GXManufacturerCollection
        extends ArrayList<GXManufacturer> implements Parcelable {
    ...
    public  boolean isUpdatesAvailable(final Context context) throws Exception {
        ...
        final AutoResetEvent completed = new AutoResetEvent(false);
    ...
    public static void readManufacturerSettings(
            final GXManufacturerCollection manufacturers, final String path, AssetManager assetManager) {
        manufacturers.clear();
       try{
          // File di = new File(path);
           String assetsFolderPath = new File("assets/"+path).getAbsolutePath();
           String fileName = "ace.obx";
           File file = new File(assetsFolderPath, fileName);
           System.out.println("Absolute Path: " + file.getAbsolutePath());
//           if (file.exists()) {
//               GXManufacturerCollection.readManufacturerSettings(items, file.getAbsolutePath());
//           } else {
...
//           }
           ...
           if (files != null) {
               for (String it : files) {
                   if (it.endsWith(".obx")) {
                       try {
                           InputStream inp= assetManager.open(path + File.separator + it);
                           manufacturers.add(parse(inp));
                       } catch (Exception e) {
        ...
        while ((event = parser.next()) != XmlPullParser.END_DOCUMENT) {
            if (event == XmlPullParser.START_TAG) {
                ...
                if (target.equalsIgnoreCase("GXManufacturer")) {
                    ...
                    if (authentication != null) {
                        authentication.setType(Authentication
