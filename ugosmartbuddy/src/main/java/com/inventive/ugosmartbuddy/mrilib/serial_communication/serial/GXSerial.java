...
import com.inventive.ugosmartbuddy.mrilib.serial_communication.common.GXSync;
import com.inventive.ugosmartbuddy.mrilib.serial_communication.common.GXSynchronousMediaBase;
import com.inventive.ugosmartbuddy.mrilib.serial_communication.common.IGXMedia2;
import com.inventive.ugosmartbuddy.mrilib.serial_communication.common.IGXMediaListener;
import com.inventive.ugosmartbuddy.mrilib.serial_communication.common.MediaStateEventArgs;
import com.inventive.ugosmartbuddy.mrilib.serial_communication.common.PropertyChangedEventArgs;
import com.inventive.ugosmartbuddy.mrilib.serial_communication.common.ReceiveEventArgs;
import com.inventive.ugosmartbuddy.mrilib.serial_communication.common.ReceiveParameters;
import com.inventive.ugosmartbuddy.mrilib.serial_communication.common.TraceEventArgs;
import com.inventive.ugosmartbuddy.mrilib.serial_communication.common.enums.MediaState;
import com.inventive.ugosmartbuddy.mrilib.serial_communication.common.enums.TraceLevel;
import com.inventive.ugosmartbuddy.mrilib.serial_communication.common.enums.TraceTypes;
...
import com.inventive.ugosmartbuddy.mrilib.serial_communication.serial.enums.AvailableMediaSettings;
...
import com.inventive.ugosmartbuddylib.R;
...
public class GXSerial implements IGXMedia2, AutoCloseable {
     ...
     */
    public GXUsbReciever mUsbReciever;
    ...
    void addPort(final UsbManager m, final UsbDevice device, final boolean notify) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
             permissionIntent = PendingIntent.getBroadcast(mContext, 0,
                    new Intent(name), PendingIntent.FLAG_IMMUTABLE);
        } else {
             permissionIntent = PendingIntent.getBroadcast(mContext, 0,
                    new Intent(name), PendingIntent.FLAG_ONE_SHOT | PendingIntent.FLAG_IMMUTABLE);
        }
            ...
            if (mPorts == null) {
                try {
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                        mContext.registerReceiver(mUsbReciever, filter2, Context.RECEIVER_NOT_EXPORTED);
                    }else {
    ...
    public final void send(final Object data) throws Exception {
        ...
        if (!WifiConnectivity.getIsWifi()) {
            ...
            synchronized (this) {
                if (WifiConnectivity.socket != null)
                    WifiConnectivity.socket.getOutputStream().write(buff);
                else
                    throw new Exception("Wifi Connection Failed.");
            }
            ...
            try {
                synchronized (this) {
                    if (WifiConnectivity.getConnectivity() == null) {
                        connectivity = new WifiConnectivity(mActivity);
                        connectivity.ConnectWifi(this);

                    } else {
                        connectivity = WifiConnectivity.getConnectivity();
                        connectivity.ConnectWifi(this);
                    }
                }
                ...
                if (WifiConnectivity.getConnectivity() == null) {
                    notifyMediaStateChange(MediaState.OPEN);
                    connectivity.setConnectivity(connectivity);
                }
    ...
    /*public Fragment properties() {
        GXPropertiesBase.setSerial(this);
        return 0;//new //GXPropertiesFragment();
    }
*/
