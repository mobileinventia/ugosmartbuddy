...
public class GXDLMSReader_4_12_23 {
    GXSerial Media;
    TraceLevel Trace;
    ...
    public GXDLMSReader_4_12_23(GXDLMSSecureClient client, GXSerial media, TraceLevel trace)
            throws Exception {
        Trace = trace;
        Media = media;
        dlms = client;
    ...
    public void readDLMSPacket(byte[] data, GXReplyData reply)
        ...
        synchronized (Media.getSynchronous()) {
            while (!succeeded) {
                Log.d("Tx:\t" , GXCommon.bytesToHex(data));
        ...
        if (reply.getError() != 0) {
            if (reply.getError() == ErrorCode.REJECTED.getValue()) {
                ...
                try{
                    readDLMSPacket(data, reply);
                }catch(Exception connectionBrokenException)
                {
        ...
        try {
            ...
            if (data.length != 0) {
                ...
                while (reply.isMoreData()) {
                    ...
                    System.out.println("Reading Data Block....");
     ...
     * @throws Exception
     */
    public void initializeConnection(String password) throws Exception, InterruptedException {
        ...
        // Parse reply.
        Log.d( "Rx: ", GXCommon.bytesToHex(reply.getData().getData()));
        ...
        if (!password.equalsIgnoreCase("1234567800000000")) {
            for (byte[] it : dlms.getApplicationAssociationRequest()) {
                readDLMSPacket(it, reply);
            }
            dlms.parseApplicationAssociationResponse(reply.getData());
    ...
    public boolean disconnect() throws Exception{
        byte[][] data = null;
        data = dlms.method("0.0.96.3.10.255", ObjectType.DISCONNECT_CONTROL, (int) 1 , 0, DataType.INT8);
        readDLMSPacket(data);
        return false;
    }
    ...
    public boolean connect() throws Exception{
        byte[][] data = null;
        data = dlms.method("0.0.96.3.10.255", ObjectType.DISCONNECT_CONTROL, (int) 2 , 0, DataType.INT8);
        readDLMSPacket(data);
        return true;
    }
     ...
     * @throws Exception
     */
    public Object readAllRowsPrepayData(final GXDLMSObject pg) throws Exception {
        GXReplyData reply = null;
        try {
            reply = new GXReplyData();
