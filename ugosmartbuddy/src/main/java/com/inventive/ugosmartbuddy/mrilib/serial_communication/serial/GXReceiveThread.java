 ...
 */
 public  class GXReceiveThread extends Thread {
    ...
    private static MeterDataListener meterDataListener;
    /**
     ...
     */
    public GXReceiveThread(final GXSerial parent, final UsbDeviceConnection conn, final UsbEndpoint input) {
        mConnection = conn;
        mInput = input;
        mParentMedia = parent;
    ...
    public static void setMeterDataListener(MeterDataListener meterDataListener1){
        meterDataListener = meterDataListener1;
    }
     ...
     */
    final public void resetBytesReceived() {
        mBytesReceived = 0;
    }
     ...
     */
    public void handleReceivedData(final byte[] buffer, final int len) {
        mBytesReceived += len;
        int totalCount = 0;
        if (mParentMedia.getIsSynchronous()) {
            ...
            if (mParentMedia.getTrace() == TraceLevel.VERBOSE) {
                mParentMedia.notifyTrace(new TraceEventArgs(TraceTypes.RECEIVED, data));
            }
            ReceiveEventArgs arg = new ReceiveEventArgs(data, mParentMedia.getPort().getPort());
            mParentMedia.notifyReceived(arg);
        }
        if(meterDataListener!=null)
            meterDataListener.onReceive("Received bytes : ", mBytesReceived+"");
